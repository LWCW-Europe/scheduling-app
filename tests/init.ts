...
if (!apiKey || !baseId) {
  throw new Error(
    "Missing Airtable config: check AIRTABLE_API_KEY and AIRTABLE_BASE_ID"
  );
}
...
async function initTestData() {
  ...
  const sessionRecords = await seedSessions(
    eventRecords[0].id,
    locationRecords.map((l) => l.id)
  );

  console.log("Seeding RSVPs...");
  const rsvpRecords = await seedRSVPs(
    guestRecords.map((u) => u.id),
    sessionRecords.map((s) => s.id)
  );
